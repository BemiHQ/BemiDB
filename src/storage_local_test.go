...
func TestCreateParquet(t *testing.T) {
	t.Run("Creates a parquet file", func(t *testing.T) {
		...
		parquetFile, _, err := storage.CreateParquet(tempDir, TEST_STORAGE_PG_SCHEMA_COLUMNS, 0, loadRows)
...
func createTestParquetFile(storage *StorageLocal, dir string) ParquetFile {
	...
	parquetFile, _, err := storage.CreateParquet(dir, TEST_STORAGE_PG_SCHEMA_COLUMNS, 0, loadRows)
	if err != nil {
		panic(err)
	}
